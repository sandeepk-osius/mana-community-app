...
export function TournamentSection({ title, badge, badgeColor, emptyText, events, onEdit, onDelete, onActivate, showActivate, onViewRegistrations, showViewRegistrations, viewingEventId, registrations, nominatedCaptains, loadingRegs, onConfirmRegistration, onConfirmCaptain, onAddParticipant, onImportParticipants }: TournamentSectionProps) {
    ...
    return (
      <div className="mt-2 bg-[#0c1220] border border-[#1e293b] rounded-xl p-4 space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#2a3a5c] pb-3 mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded bg-[#3b82f6]/20 text-[#3b82f6]">
              {viewMode === "captains" ? (nominatedCaptains?.length ?? 0) : (registrations?.length ?? 0)} {viewMode === "captains" ? "captains" : "players"}
            </span>
          </div>
              ...
              <div key={reg.id} className="flex items-center justify-between p-3 bg-[#141c2e] rounded-lg border border-[#2a3a5c]">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {reg.status === "REGISTERED" && onConfirmRegistration && (
                    <button onClick={() => onConfirmRegistration(reg.id)} className="text-[10px] px-3 py-1.5 bg-[#10b981]/10 text-[#10b981] border border-[#10b981]/30 rounded-lg hover:bg-[#10b981]/20 transition-colors flex items-center gap-1 font-medium">
        ...
        {!loadingRegs && viewMode === "captains" && nominatedCaptains && nominatedCaptains.length > 0 && (
          <div className="space-y-2">
            {nominatedCaptains.map((team, idx) => (
              <div key={team.id} className="flex items-center justify-between p-3 bg-[#141c2e] rounded-lg border border-[#2a3a5c]">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {team.captainConfirmation ? (
                    ...
                    onConfirmCaptain && (
                      <button onClick={() => onConfirmCaptain(team.id, true)} className="text-[10px] px-3 py-1.5 bg-[#f97316]/10 text-[#f97316] border border-[#f97316]/30 rounded-lg hover:bg-[#f97316]/20 transition-colors flex items-center gap-1 font-medium">
  ...
  return (
    <div className="bg-[#141c2e] border border-[#2a3a5c] rounded-xl p-4">
      ...
      <div className="space-y-2">
        {events.map(item => {
          // If the tournament has a list of related sports events, render the multi-event layout
                    ...
                    return (
                      <div key={ev.id} className="space-y-2">
                        <div className="flex items-center justify-between p-2.5 bg-[#141c2e] rounded-lg border border-[#2a3a5c]/40">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {registrationStatus && (
                              <span className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${registrationStatus === "LIVE" || registrationStatus === "REGISTRATION_OPEN" ? "bg-green-500/20 text-[#10b981]" : registrationStatus === "COMPLETED" ? "bg-blue-500/20 text-[#3b82f6]" : "bg-[#f97316]/20 text-[#f97316]"}`}>
                                {registrationStatus}
                              </span>
                            )}
                            {showViewRegistrations && onViewRegistrations && (
                              <>
                                <button onClick={() => { setViewMode("players"); onViewRegistrations(ev.id); }} className={`text-[9px] px-2 py-1 rounded transition-colors flex items-center gap-1 ${viewingEventId === ev.id && viewMode === "players" ? "bg-[#3b82f6]/20 text-[#3b82f6] border border-[#3b82f6]/30" : "border border-[#2a3a5c] text-[#94a3b8] hover:border-[#3b82f6] hover:text-[#3b82f6]"}`}>
                                  <ClipboardList className="w-2.5 h-2.5" /> {viewingEventId === ev.id && viewMode === "players" ? "Hide Players" : "View Players"}
                                </button>
                                <button onClick={() => { setViewMode("captains"); onViewRegistrations(ev.id); }} className={`text-[9px] px-2 py-1 rounded transition-colors flex items-center gap-1 ${viewingEventId === ev.id && viewMode === "captains" ? "bg-[#f97316]/20 text-[#f97316] border border-[#f97316]/30" : "border border-[#2a3a5c] text-[#94a3b8] hover:border-[#f97316] hover:text-[#f97316]"}`}>
                                  <Users className="w-2.5 h-2.5" /> {viewingEventId === ev.id && viewMode === "captains" ? "Hide Captains" : "Nominated Captains"}
                                </button>
                              </>
                            )}
                          </div>
                        ...
                        {/* Nested Registration List Panel */}
                        {showViewRegistrations && viewingEventId === ev.id && renderRegistrationPanel(ev)}
                      </div>
                    );
                  })}
                </div>
          ...
          // Single-event/fallback mapping
          const ev = item.event ? { ...item.event, id: item.event.id, name: item.name || item.event.name, tournamentId: item.id } : item;
          ...
          return (
            <div key={item.id} className="space-y-2">
              <div className="flex items-center justify-between p-3 bg-[#0c1220] rounded-xl border border-[#1e293b]">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {registrationStatus && registrationStatus !== "DRAFT" && (
                    <span className={`text-[10px] px-2 py-1 rounded ${registrationStatus === "LIVE" || registrationStatus === "REGISTRATION_OPEN" ? "bg-green-500/20 text-[#10b981]" : registrationStatus === "COMPLETED" ? "bg-blue-500/20 text-[#3b82f6]" : "bg-[#f97316]/20 text-[#f97316]"}`}>
                      {registrationStatus}
                    </span>
                  )}
                  {showViewRegistrations && onViewRegistrations && (
                    <>
                      <button onClick={() => { setViewMode("players"); onViewRegistrations(ev.id); }} className={`text-[10px] px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1 ${viewingEventId === ev.id && viewMode === "players" ? "bg-[#3b82f6]/20 text-[#3b82f6] border border-[#3b82f6]/30" : "border border-[#2a3a5c] text-[#94a3b8] hover:border-[#3b82f6] hover:text-[#3b82f6]"}`}>
                        <ClipboardList className="w-3 h-3" /> {viewingEventId === ev.id && viewMode === "players" ? "Hide Players" : "View Players"}
                      </button>
                      <button onClick={() => { setViewMode("captains"); onViewRegistrations(ev.id); }} className={`text-[10px] px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1 ${viewingEventId === ev.id && viewMode === "captains" ? "bg-[#f97316]/20 text-[#f97316] border border-[#f97316]/30" : "border border-[#2a3a5c] text-[#94a3b8] hover:border-[#f97316] hover:text-[#f97316]"}`}>
                        <Users className="w-3 h-3" /> {viewingEventId === ev.id && viewMode === "captains" ? "Hide Captains" : "Nominated Captains"}
                      </button>
                    </>
                  )}
                  {showActivate && onActivate && (
              ...
              {/* Single event Registration List Panel */}
              {showViewRegistrations && viewingEventId === ev.id && renderRegistrationPanel(ev)}
            </div>
          );
        })}
