...
import type { TournamentTypeInfo, EventInfo } from '../../../services/tournamentService';
...
export function SetupSchedule() {
  ...
  const isGroupKnockout = !!(
    ...
    (
      ...
      format === 'group_knockout' ||
      (formatKey.includes('group') && (formatKey.includes('knock') || formatKey.includes('ko')))
    ) &&
    format !== 'KNOCKOUT'
  );
  const isKnockoutOnly = !!(format && (format === 'KNOCKOUT' || (formatKey.includes('knockout') && !isGroupKnockout)));
  ...
  const [allParticipantOptions, setAllParticipantOptions] = useState<Array<{ id: string; name: string }>>([]);
  ...
  const handleEventChange = (id: string) => {
    ...
    if (ev) {
      if (ev.totalTeams > 0) setParticipants(ev.totalTeams.toString());
  ...
  const buildGroupsState = () => {
    ...
    const allParts: Array<{ id: string; name: string }> = [];
    for (let i = 1; i <= numParticipants; i++) {
      allParts.push({ id: `P${i}`, name: `Participant ${i}` });
    }
  ...
  const handleScheduleGroups = async () => {
    ...
    setGenerating(true);
    try {
      await tournamentService.createConfig(buildScheduleConfigBody());
      buildGroupsState();
      setPlayoffsGenerated(false);
      setScheduleGenerated(true);
      setShowSetup(false);
      setToast('Group schedule generated! Configure proceeders, then click Generate Schedule for playoffs.');
    } catch (err: any) {
  ...
  const handleGeneratePlayoffs = async () => {
    ...
    try {
      ...
      setToast('Knockout bracket generated: semi-finals and final');
    } catch (err: any) {
      setToast('Failed: ' + (err.message || 'Unknown error'));
    } finally {
      setGenerating(false);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (isKnockoutOnly) {
        setGeneratedGroups([]);
        setPlayoffsGenerated(true);
        setToast('Knockout schedule generated successfully!');
      } else {
        setPlayoffsGenerated(false);
  ...
  const getPlayoffMatches = () => {
    ...
    let playoffStartDate = groupEnd.date;
    let playoffStartTime = addMinutesToTime(groupEnd.time, breakMins);

    const draft = buildPlayoffBracket({
      numGroups,
      proceedersPerGroup,
    ...
    return applyPlayoffOverrides(draft, matchOverrides);
  };
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      await tournamentService.createConfig({
        ...buildScheduleConfigBody(),
        status: 'DRAFT',
      });
      setToast('Schedule saved as draft');
    } catch (err: any) {
  ...
  const handleSavePublish = async () => {
    ...
    try {
      await tournamentService.createConfig(buildScheduleConfigBody());
      setToast('Schedule saved and published');
    } catch (err: any) {
      setToast('Publish failed: ' + (err.message || 'Unknown error'));
    } finally {
      setSavingSchedule(false);
      setTimeout(() => setToast(null), 3000);
    }
  ...
  // Reusable styles
  const inputCls = 'w-full bg-[#0f1729] border border-[#2a3a5c] rounded-lg px-4 py-2.5 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00] transition-colors [color-scheme:dark]';
  ...
  return (
    <div className="space-y-5">
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 px-5 py-3 rounded-lg shadow-xl font-medium text-sm bg-[#c8ff00] text-black">{toast}</div>
      )}

      {/* Header */}
      <div className="border border-dashed border-[#c8ff00]/40 rounded-xl p-4 md:p-6">
        <h2 className="text-xl md:text-2xl font-bold text-center text-[#c8ff00] tracking-wider mb-2">SETUP SCHEDULE</h2>
        <div className="flex justify-center">
          <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs px-4 py-2 rounded-lg">
            <AlertTriangle className="w-4 h-4 shrink-0" />
            <span>This is a sample schedule and will not be saved. Please finalize participants to save your tournament schedule.</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className={cardCls}>
          ...
          <div className="space-y-4">
            {/* Format */}
            <div>
              <label className={labelCls}>Format <span className="text-red-400">*</span></label>
              <select value={format} onChange={e => handleFormatChange(e.target.value)} className={inputCls}>
                <option value="">Select Schedule Type</option>
                {types.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
              </select>
              {isGroupKnockout && (
                <p className="text-[11px] text-[#c8ff00]/80 mt-1.5">
            ...
            {/* Participants */}
            <div>
              <label className={labelCls}>Number of Participants <span className="text-red-400">*</span></label>
              <input type="number" min="2" max="128" value={participants} onChange={e => setParticipants(e.target.value)} className={inputCls} placeholder="Number of Participants" />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full transition-colors ${thirdPlace ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
              >
              ...
              <div className="flex flex-wrap justify-center gap-4 mt-2">
                {SEEDING_OPTIONS.map(opt => (
                  <label key={opt.value} className="flex items-center gap-2 cursor-pointer text-sm text-slate-300">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${seedingOrder === opt.value ? 'border-[#c8ff00]' : 'border-[#2a3a5c]'}`}>
                      {seedingOrder === opt.value && <div className="w-2 h-2 rounded-full bg-[#c8ff00]" />}
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-xs border border-[#c8ff00]/40 text-[#c8ff00] px-3 py-1.5 rounded-lg hover:bg-[#c8ff00]/10 transition-colors font-semibold flex items-center gap-1"
              >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setPtsWin(val);
                    setStandingsConfig(prev => ({ ...prev, normalWin: val }));
                  }}
                  className={inputCls}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    const val = e.target.value;
                    setPtsDraw(val);
                    setStandingsConfig(prev => ({ ...prev, tiePoints: val }));
                  }}
                  className={inputCls}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={e => {
                    const val = e.target.value;
                    setPtsLoss(val);
                    setStandingsConfig(prev => ({ ...prev, normalLoss: val }));
                  }}
                  className={inputCls}
                />
              </div>
              ...
              <button
                ...
                className={`text-xs border px-3 py-1.5 rounded-lg transition-colors font-semibold flex items-center gap-1 ${
                  participantsScoring.enabled
                    ? 'border-[#c8ff00] text-[#c8ff00] hover:bg-[#c8ff00]/10'
                    : 'border-[#2a3a5c] text-slate-400 hover:border-[#c8ff00]/40 hover:text-[#c8ff00]'
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className={cardCls}>
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`relative w-9 h-5 rounded-full transition-colors ${showDatePicker ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
              >
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              <input type="checkbox" checked={limitMatchesPerDay} onChange={e => setLimitMatchesPerDay(e.target.checked)} className="w-4 h-4 accent-[#c8ff00] rounded" id="limitPerDay" />
              <label htmlFor="limitPerDay" className="text-sm text-slate-300 cursor-pointer">Limit Participant Matches Per Day</label>
              {limitMatchesPerDay && (
                <input type="number" min="1" max="10" value={maxMatchesPerDay} onChange={e => setMaxMatchesPerDay(e.target.value)} className="w-16 bg-[#0f1729] border border-[#2a3a5c] rounded-lg px-2 py-1 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00]" />
              )}
            </div>
          </div>
        </div>
        ...
        <div className={cardCls}>
          <div className="flex items-center justify-between mb-5">
            ...
            <button className="text-xs border border-[#c8ff00]/40 text-[#c8ff00] px-3 py-1.5 rounded-lg hover:bg-[#c8ff00]/10 transition-colors font-semibold flex items-center gap-1">
            ...
            {venues.length === 0 ? (
              ...
              <div key={v.id} className="border border-[#2a3a5c] rounded-lg overflow-hidden">
                <div className="flex items-center gap-3 p-3 bg-[#0f1729]/50 cursor-pointer" onClick={() => toggleVenue(v.id)}>
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-colors shrink-0 ${selectedVenues.includes(v.id) ? 'border-[#c8ff00] bg-[#c8ff00]' : 'border-[#2a3a5c]'}`}>
                ...
                {expandedVenue === v.id && (
                  <div className="px-4 py-3 bg-[#0f1729]/30 border-t border-[#2a3a5c] text-xs space-y-1.5">
                    ...
                    <div className="pt-1">
                      <button className="text-[11px] border border-[#2a3a5c] text-slate-400 px-2 py-1 rounded hover:border-[#c8ff00] hover:text-[#c8ff00] transition-colors flex items-center gap-1">
            ...
            <div className="flex justify-center gap-1 mt-2">
              {VENUE_ASSIGN_OPTIONS.map(opt => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${venueAssignType === opt ? 'bg-[#c8ff00] text-black shadow-md' : 'bg-[#0f1729] border border-[#2a3a5c] text-slate-400 hover:border-slate-500'}`}
                >
                  {opt}
                </button>
              ))}
      ...
      {!scheduleGenerated && format && (
        <div className="text-center pt-2 pb-4">
          <button
            ...
            className="px-8 py-3 bg-[#c8ff00] hover:bg-[#a3cc00] text-black font-bold rounded-lg transition-colors text-sm tracking-wide disabled:opacity-50 shadow-lg shadow-[#c8ff00]/10"
          >
            {generating
      ...
      {scheduleGenerated && (
        <div className="space-y-5">
          ...
          {/* Warning Banner */}
          <div className="flex flex-col md:flex-row gap-2 w-full text-center justify-center md:justify-between items-center bg-red-500/10 border border-red-500/30 text-red-400 rounded-lg px-4 py-3">
            <span className="text-xs md:text-sm">Participants for this division are yet to be finalized. Finalize from the Participants tab.</span>
            <button className="px-4 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs md:text-sm font-semibold rounded-full transition-colors">
              Finalize Participants
            </button>
          </div>

          {/* Collapsible Setup Schedule */}
          <div className="p-2 md:p-4 border border-dashed border-[#c8ff00]/40 rounded-xl">
            <div className="flex flex-col gap-3 md:gap-5">
              <h4 className="text-base md:text-xl font-bold text-center text-[#c8ff00]">SETUP SCHEDULE</h4>
              <div className="text-center">
                <button
                  onClick={() => setShowSetup(!showSetup)}
                  className="px-4 py-1.5 border border-[#c8ff00]/40 text-[#c8ff00] text-xs font-semibold rounded-full hover:bg-[#c8ff00]/10 transition-colors inline-flex items-center gap-1"
                >
                  {showSetup ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  {showSetup ? 'Hide Setup' : 'View Setup'}
                </button>
              </div>
              {showSetup && (
                <div className="text-xs text-slate-400 space-y-1 bg-[#0f1729]/50 border border-[#2a3a5c] rounded-lg p-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div><span className="text-slate-500">Format:</span> <span className="text-slate-200 font-medium">{types.find(t => t.id.toString() === format)?.name || format}</span></div>
                    <div><span className="text-slate-500">Participants:</span> <span className="text-slate-200 font-medium">{participants}</span></div>
                    {isGroupKnockout && (
                      <div><span className="text-slate-500">Groups:</span> <span className="text-slate-200 font-medium">{numberOfGroups}</span></div>
                    )}
                    <div><span className="text-slate-500">Seeding:</span> <span className="text-slate-200 font-medium">{seedingOrder}</span></div>
                    <div><span className="text-slate-500">3rd Place:</span> <span className="text-slate-200 font-medium">{thirdPlace ? 'Yes' : 'No'}</span></div>
                    <div><span className="text-slate-500">Pts Win:</span> <span className="text-slate-200 font-medium">{ptsWin}</span></div>
                    <div><span className="text-slate-500">Pts Draw:</span> <span className="text-slate-200 font-medium">{ptsDraw}</span></div>
                    <div><span className="text-slate-500">Pts Loss:</span> <span className="text-slate-200 font-medium">{ptsLoss}</span></div>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="border border-dashed border-[#c8ff00]/40 rounded-xl bg-[#0f1729]/30 px-3 pb-4 relative">
            {/* Sample Ribbon */}
            <div className="flex justify-center -mt-3 mb-4">
              <span className="px-4 py-1 bg-emerald-500 text-white text-xs font-bold rounded-full tracking-wider shadow-lg">Sample</span>
            </div>
            ...
            <div className="flex flex-col mt-4 gap-4">
              <div className="flex flex-wrap gap-4 justify-center">
                {generatedGroups.map((group, gIdx) => (
                  <div key={group.name} className="bg-[#141c2e] border border-[#2a3a5c] rounded-xl p-3 overflow-hidden w-[18rem]">
                    <table className="table-auto text-center border-collapse w-full">
                      <thead>
                        <tr>
                          <th className="pb-3 text-sm font-bold text-[#c8ff00]">{group.name}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {/* Participant List */}
                              ...
                              {group.participants.map((p, pIdx) => (
                                <div key={p.id + pIdx} className="flex items-center w-full gap-2">
                                  ...
                                  <select
                                    ...
                                    className="flex-1 bg-[#0f1729] border border-[#2a3a5c] rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00] transition-colors"
                                  >
                        ...
                        <tr>
                          <td className="text-left">
                            <div className="flex flex-col gap-3 border border-[#2a3a5c] rounded-lg p-3">
                              <div className="flex flex-col gap-1">
                                ...
                                <input
                                  ...
                                  className="w-full bg-[#0f1729] border border-[#2a3a5c] rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00] transition-colors"
                                />
                              </div>
                              <div className="flex flex-col gap-1">
                                ...
                                {isGroupKnockout && (
                                  <button
                                    ...
                                    className="mt-2 w-full py-1.5 border border-[#c8ff00]/40 text-[#c8ff00] text-xs font-semibold rounded-lg hover:bg-[#c8ff00]/10 transition-colors cursor-pointer text-center"
                                  >
                                    Select Proceeders
                                  </button>
                                )}
                              ...
                              <div className="flex flex-col gap-1 mt-4">
                                <div className="flex gap-2 items-center justify-between">
                                  ...
                                  <button
                                    ...
                                    className={`relative w-9 h-5 rounded-full transition-colors shrink-0 ${group.limitMatches ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                                  >
                                ...
                                {group.limitMatches && (
                                  <input
                                    ...
                                    className="w-full mt-1 bg-[#0f1729] border border-[#2a3a5c] rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00] transition-colors"
                              ...
                              {gIdx === 0 && generatedGroups.length > 1 && (
                                <div className="flex justify-between items-center mt-2 pt-2 border-t border-[#2a3a5c]">
                                  ...
                                  <button
                                    ...
                                    className="px-3 py-1 border border-[#c8ff00]/40 text-[#c8ff00] text-xs font-semibold rounded-lg hover:bg-[#c8ff00]/10 transition-colors"
                                  >
                                    Apply
                                  </button>
                                </div>
              ...
              <div className="flex flex-col gap-4 mt-4 items-center">
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${playoffThirdPlace ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                    >
                  ...
                  <div className="flex flex-col gap-4 items-center justify-center">
                    ...
                    <div className="flex flex-wrap gap-4 justify-center items-center">
                      {SEEDING_OPTIONS.map(opt => (
                        <label key={opt.value} className="flex items-center gap-2 cursor-pointer text-sm text-slate-300">
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${playoffSeedingOrder === opt.value ? 'border-[#c8ff00]' : 'border-[#2a3a5c]'}`}>
                            {playoffSeedingOrder === opt.value && <div className="w-2 h-2 rounded-full bg-[#c8ff00]" />}
                          </div>
                ...
                <div key={'fixtures-' + group.name} className="flex flex-col gap-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {matches.map((match, mIdx) => {
                      ...
                      return (
                        <div key={mIdx} className="bg-[#141c2e] border border-[#2a3a5c] rounded-xl overflow-hidden relative pt-1 shadow-md hover:border-[#c8ff00]/40 transition-colors">
                          ...
                          <div className="flex flex-col gap-1 px-3.5 py-2 text-xs text-slate-400 border-b border-[#2a3a5c]/40 bg-[#0f1729]/30">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1">
                                <i className="pi pi-calendar text-[#c8ff00] text-sm"></i>
                              ...
                              <div className="flex items-center gap-1">
                                <i className="pi pi-clock text-[#c8ff00]"></i>
                                <span className="font-bold text-slate-200">{match.time}</span>
                              </div>
                            </div>
                            <div className="flex items-center gap-1 text-[11px] text-slate-400/80">
                              <i className="pi pi-map-marker text-[#c8ff00]"></i>
                              <span>{match.venue}</span>
                              <span className="text-[#c8ff00] font-semibold"> - {match.court}</span>
                            </div>
                          </div>
                          ...
                          <div className="p-3.5 flex flex-col justify-between h-[calc(100%-6.5rem)]">
                            <div className="flex items-center justify-between bg-[#0f1729]/40 rounded-lg p-2.5 border border-[#2a3a5c]/40">
                              ...
                              <div className="flex justify-center w-[16%]">
                                <span className="text-[10px] bg-[#c8ff00]/10 text-[#c8ff00] px-2 py-0.5 rounded font-bold border border-[#c8ff00]/20">VS</span>
                              </div>
                            ...
                            <div className="flex justify-between items-center mt-3 pt-3 border-t border-[#2a3a5c]/30">
                              ...
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="px-2.5 py-1 border border-[#c8ff00]/40 text-[#c8ff00] text-xs font-semibold rounded-lg hover:bg-[#c8ff00]/10 transition-colors flex items-center gap-1 cursor-pointer"
                                >
          ...
          {!showScheduleSaveFooter && (
            <div className="flex gap-2 justify-center items-center mt-4">
              ...
              <span className="text-xs md:text-sm text-red-400">
                This is sample schedule and will not be saved. Please finalize participants to save your tournament schedule.
              </span>
            </div>
          )}
        </div>
          ...
          <div className="bg-[#141c2e] border border-[#2a3a5c] rounded-xl w-full max-w-[40rem] flex flex-col shadow-2xl relative my-8 max-h-[90vh]">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex flex-row justify-between items-center py-2">
                  ...
                  <div className="flex justify-center text-center w-[10%]">
                    <div className="text-center font-bold text-xl text-[#c8ff00] tracking-wider drop-shadow-md">VS</div>
                  </div>
                  <div className="flex justify-center w-[45%]">
                    <div className="w-16 h-16 rounded-full bg-slate-800 border-2 border-[#2a3a5c] overflow-hidden flex items-center justify-center shadow-lg">
                      <img
                ...
                <div className="flex items-center gap-2.5 py-1">
                  <input
                    ...
                    className="w-4 h-4 accent-[#c8ff00] rounded cursor-pointer shrink-0"
                  />
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-[#2a3a5c] shrink-0 bg-[#0f1729]/50 rounded-b-xl">
              ...
              <button
                ...
                className="px-5 py-2 bg-[#c8ff00] hover:bg-[#a3cc00] text-black font-bold rounded-lg text-sm transition-colors shadow-lg shadow-[#c8ff00]/10"
              >
                Submit
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-[#141c2e] border border-[#2a3a5c] rounded-xl w-full max-w-[50rem] flex flex-col shadow-2xl relative my-8 max-h-[90vh]">
            ...
            <div className="p-6 space-y-6 overflow-y-auto scrollbar-thin scrollbar-thumb-[#2a3a5c]">
              ...
              {/* 1. Points System */}
              <div className="space-y-4">
                <h6 className="text-base font-bold text-[#c8ff00] border-b border-[#2a3a5c]/50 pb-2">Points System</h6>
                ...
                <div className="flex items-center justify-between bg-[#0f1729]/50 border border-[#2a3a5c] rounded-lg p-4">
                  <div className="space-y-1 pr-4">
                    ...
                    <p className="text-xs text-slate-400 flex items-center gap-1.5">
                      <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#c8ff00]"></span>
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalAllowVariablePoints ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                  >
                      ...
                      <tr className="hover:bg-[#0f1729]/20 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="w-28 bg-[#0f1729] border border-[#2a3a5c] rounded px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00] disabled:opacity-40 disabled:cursor-not-allowed"
                            placeholder="Points"
                          />
                        </td>
                        <td className="px-4 py-3">
                      ...
                      <tr className="hover:bg-[#0f1729]/20 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="w-28 bg-[#0f1729] border border-[#2a3a5c] rounded px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00]"
                            placeholder="Points"
                          />
                        </td>
                        <td className="px-4 py-3">
                      ...
                      <tr className="hover:bg-[#0f1729]/20 transition-colors">
                        ...
                        <td className="px-4 py-3.5 flex justify-end">
                          <button
                            ...
                            className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalCustomPoints ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                          >
                      ...
                      <tr className="hover:bg-[#0f1729]/20 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            className="w-36 bg-[#0f1729] border border-[#2a3a5c] rounded px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00]"
              ...
              <div className="space-y-3">
                <h6 className="text-base font-bold text-[#c8ff00] border-b border-[#2a3a5c]/50 pb-2">Standings Display Settings</h6>
                <p className="text-xs text-slate-400 flex items-center gap-1.5">
                  <span className="inline-block w-1.5 h-1.5 rounded-full bg-[#c8ff00]"></span>
                  ...
                  {displayParams.map(({ key, label }) => (
                    <div key={key} className="flex items-center justify-between p-3.5 bg-[#141c2e] hover:bg-[#1e293b]/20 transition-colors">
                      ...
                      <button
                        ...
                        className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalDisplaySettings[key] ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                      >
              ...
              <div className="space-y-3">
                <h6 className="text-base font-bold text-[#c8ff00] border-b border-[#2a3a5c]/50 pb-2">Additional Parameters</h6>
                        ...
                        <tr key={param.id} className="hover:bg-[#0f1729]/20 transition-colors">
                          <td className="px-4 py-2">
                            <input
                              ...
                              className="w-full bg-[#0f1729] border border-[#2a3a5c] rounded px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-[#c8ff00]"
                              placeholder="Points"
                              required
                            />
                          </td>
                      ...
                      <tr className="bg-[#0f1729]/30">
                        ...
                        <td className="px-4 py-2 text-center">
                          <button
                            ...
                            className="p-1.5 text-[#c8ff00] hover:text-[#a3cc00] hover:bg-[#c8ff00]/10 rounded transition-colors"
                            title="Add Parameter"
                          >
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between gap-4 p-4 bg-[#0f1729]/50 border border-[#2a3a5c] rounded-lg">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalAdminApproval ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                  >
                ...
                <div className="flex items-center justify-between gap-4 p-4 bg-[#0f1729]/50 border border-[#2a3a5c] rounded-lg">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalCanPublish ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                  >
                ...
                <div className="flex items-center justify-between gap-4 p-4 bg-[#0f1729]/50 border border-[#2a3a5c] rounded-lg">
                  ...
                  <button
                    ...
                    className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${modalOpponentApproval ? 'bg-[#c8ff00]' : 'bg-[#2a3a5c]'}`}
                  >
            ...
            <div className="flex items-center justify-between p-5 border-t border-[#2a3a5c] shrink-0 bg-[#0f1729]/50 rounded-b-xl">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2 bg-[#c8ff00] hover:bg-[#a3cc00] text-black font-bold rounded-lg text-sm transition-colors shadow-lg shadow-[#c8ff00]/10 flex items-center gap-1.5"
                >
